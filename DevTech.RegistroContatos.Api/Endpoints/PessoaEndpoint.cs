...
public static class PessoaEndpoint
{
    public static RouteGroupBuilder MapPessoaEndpoints(this WebApplication app)
    {
        ...
        grupo.MapGet("/data_nascimento/{dtInicio}-{dtTermino}", async (PessoaRepositorio repositorio, string dtInicio, string dtTermino) =>
        {
            try
            {
                return await repositorio.ListarPorDataNascimento(Convert.ToDateTime(dtInicio), Convert.ToDateTime(dtTermino));
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
